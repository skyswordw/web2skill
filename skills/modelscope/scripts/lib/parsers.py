...
def build_search_models_output(
    ...
    return SearchModelsOutput(
        query=query,
        total_count=(
            _as_int(model_block.get("TotalCount"))
            or _as_int(data_block.get("Total"))
            or _as_int(data_block.get("TotalCount"))
        ),
        page=page,
...
def normalize_search_results(payload: dict[str, Any]) -> list[ModelSearchItem]:
    data_block = _data_block(payload)
    model_block = _dict_from(data_block.get("Model"))
    if not model_block and isinstance(data_block.get("Models"), list):
        model_block = data_block
    items: list[ModelSearchItem] = []
    for entry in _list_from(model_block.get("Models")):
        model_payload = _as_dict(entry)
        ...
        items.append(
            ModelSearchItem(
                ...
                or _as_str(model_payload.get("Summary")),
                task_names=_task_names(model_payload.get("Tasks") or model_payload.get("Task")),
                ...
                or _as_int(model_payload.get("DownloadCount")),
                stars=_as_int(model_payload.get("Stars")) or _as_int(model_payload.get("Likes")),
                updated_at=parse_unix_timestamp(model_payload.get("LastUpdatedTime"))
                or parse_iso_datetime(model_payload.get("UpdatedAt")),
...
def normalize_model_overview(payload: dict[str, Any]) -> ModelOverview:
    ...
    return ModelOverview(
        ...
        stars=_as_int(source.get("Stars")) or _as_int(source.get("Likes")),
        revision=_as_str(source.get("Revision")),
        license=_as_str(source.get("License")),
        tasks=_task_names(source.get("Tasks") or source.get("Task")),
        ...
        or parse_iso_datetime(source.get("UpdatedAt")),
        organization=_organization_from_payload(_as_dict(source.get("Organization"))),
...
def normalize_repo_files(
    ...
    data_block = _data_block(payload)
    for entry in _list_from(data_block.get("Files")):
        file_payload = _as_dict(entry)
        ...
        files.append(
            ModelFileEntry(
                path=path,
                name=_as_str(file_payload.get("Name")) or path.rsplit("/", 1)[-1],
...
def normalize_account_profile(payload: dict[str, Any]) -> AccountProfile:
    ...
    source = _as_dict(data) or payload
...
def normalize_token_detail(payload: dict[str, Any], *, token_id: int) -> TokenDetail:
    ...
    for raw_token in _list_from(data.get("SdkTokens")):
        token_payload = _as_dict(raw_token)
        if token_payload is None or _as_int(token_payload.get("Id")) != token_id:
            continue
        name = _as_str(token_payload.get("SdkTokenName"))
...
def normalize_create_token_output(payload: dict[str, Any], *, token_id: int) -> CreateTokenOutput:
    return CreateTokenOutput(**normalize_token_detail(payload, token_id=token_id).model_dump())
...
def _string_list(value: Any) -> list[str]:
    return [item for item in _list_from(value) if isinstance(item, str) and item]
